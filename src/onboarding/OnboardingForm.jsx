...
const OnboardingForm = ({ onClose }) => {
  ...
  const handleEducationTypeSubmit = (info) => {
    setEducationInfo(info);
    localStorage.setItem("educationType", info.educationType);
    if (info.educationType === "school") {
      localStorage.setItem("boardType", info.boardType);
    }
    nextStep();
  };
    ...
    <div className="mb-8">
      ...
      <div>
        <ol className="flex items-center justify-between gap-2 text-xs font-medium text-gray-500 sm:gap-4">
          {["Type", "Campus", "Branch", "Admin", "Review"].map((label, index) => (
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
      <div className="bg-white p-8 rounded-lg shadow-xl md:w-[30vw] w-[90vw] ">
        ...
        {step === 1 && <EducationTypeSelection onNext={handleEducationTypeSubmit} />}
        {step === 2 && <CreateCampus onNext={nextStep} educationInfo={educationInfo} />}
        {step === 3 && <CreateBranch onNext={nextStep} onPrevious={prevStep} />}
        {step === 4 && <CreateRole onNext={nextStep} onPrevious={prevStep} />}
        {step === 5 && (
          <ReviewPage
            onPrevious={prevStep}
            onSubmit={() => {
