...
const EmployeeUpload = ({ onEmployeesUpload }) => {
	...
	const handleFileUpload = async (event) => {
		...
		reader.onload = async (e) => {
			try {
				...
				const result = await onEmployeesUpload(formattedData);
				if (result && result.success === false) {
					throw new Error(result.message || "Failed to upload employees");
				}
				setToast({
					message: result.message || "Employees uploaded successfully!",
					type: "success",
				});
			} catch (error) {
	...
	return (
		<div className="mb-2 w-full">
			<label
				htmlFor="file-upload"
				className={`flex items-center justify-between bg-emerald-700 text-white w-full py-2 px-4 rounded-md cursor-pointer hover:bg-emerald-600 transition-colors duration-300 ${
